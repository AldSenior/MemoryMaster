...
export const NumbersOrdersGame = ({ difficult, gameindex }) => {
    ...
    const [StatickMassHistory, setStatickMassHistory] = useAtom(atomStatickMassHistory);
    useEffect(() => {
        const currentdate = new Date();
        const datetime = currentdate.toLocaleString("ru-ru");
    ...
    const mass = Array(difficult)
        .fill(0)
        .map((item, i) => {
            return <Block order={order} select={select} setSelect={setSelect} i={i + 1} key={i} setTime={setTime} />;
        });
    const shuffleCards = () => {
    ...
    return (
        <div className={style["game"]}>
            <div className={style["timer"]}>
                {order < difficult + 1 ? <p className={style["order"]}>Текущее число:{order}</p> : <p className={style["order"]}>Победа!</p>}
