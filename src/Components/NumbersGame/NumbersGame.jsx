...
const Ansxzc = ({answer}) => {
    return <div className={style["answer"]}><span>{answer}</span></div>
}
const Noasnwer = () =>{
...
export const NumbersGmae = () => {
    ...
    const massAnswer = Array(answer).fill(0).map((item,index)=>{
        return <Ansxzc answer={index+1}/>
    })
    const massNoAnswer = Array(25-answer).fill(0).map((item,index)=>{
        return <Noasnwer/>
    })
    const shuffleCards = () => {
        const suhhleMass = [...massAnswer, ...massNoAnswer]
        suhhleMass.sort(() => Math.random() - 0.5)
    ...
    return (
        <div className={style["game"]}>
            ...
            <p className={style["succes"]}><span>0</span><span>0</span> </p>
        </div>
